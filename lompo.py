...
class Lompo(tools.Module):
    ...
    def fit_model(self, itters):
        for itter in range(itters):
            data = next(self._dataset)
            self._model_train_step(data)
            if itter % self._c.save_every == 0:
                self.save_model(self._c.logdir / 'model_step_{}'.format(itter))
    ...
    def _agent_train_step(self, data):
        ...
        with tf.GradientTape() as q_tape:
            ...
            target_q_values = tf.reduce_min([self._target_qf1(tf.concat([next_obs, new_next_actions], axis = -1)),
                                             self._target_qf2(tf.concat([next_obs, new_next_actions], axis = -1))], axis = 0)
    ...
    def _generate_real_data(self, batches = 1):
        for _ in range(batches):
            data = next(self._dataset)
            if not self._c.pcont:
                data['terminal'] = tf.zeros_like(data['reward'])
            embed = self._encode(data)
            post, prior = self._dynamics.observe(embed, data['action'])
            feat = self._dynamics.get_feat(post)
          
            for i in range(len(feat)):
                obs           = tf.boolean_mask(feat[i], data['mask'][i]).numpy()[:-1]
                action        = tf.boolean_mask(data['action'][i], data['mask'][i]).numpy()[1:]
                next_obs      = tf.boolean_mask(feat[i], data['mask'][i]).numpy()[1:]
                reward        = tf.boolean_mask(data['reward'][i], data['mask'][i]).numpy()[1:]
                terminal      = tf.boolean_mask(data['terminal'][i], data['mask'][i]).numpy()[1:]
                
                self.latent_buffer.add_samples(obs, 
                                               action, 
                                               next_obs, 
                                               reward, 
                                               terminal,
    ...
    def _generate_latent_data(self, data):
        ...
        for index in range(self._c.horizon):
            ...
            elif self._c.penalty_type == None:
...
def episode_itterator(datadir, config):
    while True:
